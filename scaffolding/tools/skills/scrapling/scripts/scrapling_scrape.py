...
def main() -> None:
    ...
    p.add_argument(
        "--insecure",
        action="store_true",
        help="Disable TLS certificate verification for Fetcher mode when local CA chains are broken",
    )
    ...
    if args.mode == "fetcher":
        with FetcherSession(impersonate="chrome") as session:
            page = session.get(url, stealthy_headers=True, verify=not args.insecure)
