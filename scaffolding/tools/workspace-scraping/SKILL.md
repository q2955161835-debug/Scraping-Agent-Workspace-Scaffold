...
- Before dispatching subagents, create or update the target project's `doc/任务清单.md` and register URLs, files, source channels, dedupe keys, status, output locations, and failure items.
...
5. Create `doc/任务清单.md` and use dedupe keys to prevent repeated crawling of the same page, attachment, API endpoint, or literature file.
6. Build a variable-channel matrix and assign channel subagents by source type: official source, official news, third-party news, literature, reverse calculation, weak-source verification, and final aggregation.
7. Inject the matching subagent prompt template from `references/subagent-prompt-templates.md` before dispatching each subagent.
8. Store raw structured results in `原始数据/`; store downloaded files or snapshots in `原始文件/`; store temporary experiments in `try/`.
9. Put the newest cleaned dataset in the target project root and archive replaced versions in `旧数据/`.
...
- Read `references/subagent-prompt-templates.md` before dispatching subagents so prompts can be injected by source quality level and task goal.
